...
def retrieveInfoFromNmap():
    for ip in sorted(elems.keys()):
        if elems[ip].name == "Unknown":
            ...
            output = subprocess.run(f"nmap {ip}")
            print(f"Output del comando {output}\n")
...
def constructJSON():
    ...
    for ip in sorted(elems.keys()):
        if first:
            nodes += '{"id":"' + ip + '", "label":"' + elems[ip].name + '","x":0,"y":0,"size":1,"mac":"' + elems[
                ip].mac + '"}'
            first = False
        else:
...
def sniff(_timeout):
    ...
    try:
        ...
        cap = pyshark.LiveCapture(interface='eth0', display_filter='cdp or lldp')
        ...
        while not captured:
            ...
            cap.sniff(packet_count=1, timeout=2)
            if cap:
                print("FOUND!", end="\n")
                captured = True
        ...
        if cap:
            ...
            print(f"time expired, timeout setted to {_timeout}")
    finally:
        cap.eventloop.close()
